...
def build(args):
    ...
    initialize_default_storage_pool(args)
    base_client_image=create_image(args, PACKER_FOCAL_FILE, image_type="client")
    #base_client_image="https://mega.nz/file/m9oUwIAI#SQVBXVY8geqy5_hpghX3fkTDNbMse_-uo2JTxe0_S-s"
    base_edge_image=create_image(args, PACKER_EDGE_FILE, image_type="edge")
    #base_edge_image="https://mega.nz/file/L9AmSAxY#dtBXmiqnSXTKgLBYbBL4AieH-35Wp9P9zKVkMk9wjPs"
...
def initialize_default_storage_pool(args):
    """
    Function to initialize the default storage pool in the KVM hypervisor to create the images
    ...
    try:
        ...
        LOGGER.log.info("Initialize the KVM Hypervisor with default storage pool to create packer images")
        LOGGER.log.info("Configuring the Packer SSH access")
        cmd = f'sudo iptables -t nat -A PREROUTING -m comment --comment "packer-ssh" -i {default_int_name} -p tcp --dport 1011 -j DNAT --to-destination {packer_mgmt_ip}:22'
...
def prepare_setup_json(args):
    ...
    try:
        ...
        if not args.flexi_manage:
            ...
            fleximanage = "https://manage.flexiwan.com"
        else:
            LOGGER.log.info(f"Fleximanage IP is {args.flexi_manage}")
            fleximanage = args.flexi_manage
        config_file = os.path.join(TESTBED_CONFIG_PATH, args.topo, 'config', '3-site.json')
